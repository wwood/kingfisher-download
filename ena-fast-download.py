...
if __name__ == '__main__':
    ...
    if len(ftp_urls) == 0:
        ...
        logging.info("Found {} FTP URLs for download: {}".format(len(ftp_urls), ", ".join(ftp_urls)))

    if len(ftp_urls) == 1:
        if bool(args.forward_only) or bool(args.reverse_only):
            warnings.warn("Specified --forward_only or --reverse_only but only a single read set found.  Bypassing this option. ")
    else:
        if bool(args.forward_only):
